...
//        populateRequest();
  ...
  {
      DefaultTableModel model = (DefaultTableModel) tblNGORequest.getModel();
      model.setRowCount(0);
      for(WorkRequest r : network.getQueueOphan().getWorkRequestList())
      {
          if(r instanceof OrphaneRequest)
          {
                OrphaneRequest ophan = (OrphaneRequest) r;        
                Object[] row = new Object[5];
                row[0]=ophan;
                row[1]=ophan.getOrphane();
                row[2]=ophan.getSender().getAddress().get(0);
                row[3]=ophan.getNumberOfFood();
                row[4]=ophan.getEnterprise();
                model.addRow(row);
          }
